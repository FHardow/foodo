...
func TestNew_Success(t *testing.T) {
	id := uuid.New()
	u, err := user.New(id, "Alice", "alice@example.com", "+1234")
...
func TestNew_PhoneOptional(t *testing.T) {
	u, err := user.New(uuid.New(), "Bob", "bob@example.com", "")
	require.NoError(t, err)
	assert.Equal(t, "", u.Phone())
}

func TestNew_MissingID(t *testing.T) {
	u, err := user.New(uuid.Nil, "Alice", "alice@example.com", "")
	assert.Nil(t, u)
	require.Error(t, err)
...
func TestNew_MissingName(t *testing.T) {
	u, err := user.New(uuid.New(), "", "alice@example.com", "")
	assert.Nil(t, u)
	require.Error(t, err)
	assert.True(t, domerrors.Is(err, domerrors.ErrBadRequest))
}
...
func TestNew_UniqueIDs(t *testing.T) {
	...
	u1, _ := user.New(id1, "A", "a@example.com", "")
	u2, _ := user.New(id2, "B", "b@example.com", "")
...
func TestUpdateContact_Success(t *testing.T) {
	u, _ := user.New(uuid.New(), "Alice", "alice@example.com", "111")
...
func TestUpdateContact_MissingName(t *testing.T) {
	u, _ := user.New(uuid.New(), "Alice", "alice@example.com", "")
